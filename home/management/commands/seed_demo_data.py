...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # Guard: only seed when explicitly enabled
        # (avoid accidental prod wipes)
        if (
            os.environ.get("SEED_DEMO_DATA", "").lower()
        ...
        if churches_count == 0:
            ...
            self.stdout.write(
                f"Churches already present ({churches_count}); "
                "skipping import."
            )
        ...
        if events_count == 0:
            ...
            self.stdout.write(
                f"Events already present ({events_count}); "
                "skipping sample events."
            )
